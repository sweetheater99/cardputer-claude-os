...
- **(c) DND / quiet-hours gate.** Implement the already-specced `dnd` (`mcp/README.md:229`, heartbeat + `ask` `dnd` resolution): a user-set quiet-hours window + a toggle key. `notify` and `ask` honor DND (return `dnd`); **`confirm` always rings** regardless of DND — a destructive op must wait for a real human decision, and if the user is asleep it simply times out → agent stops (fail-closed), never auto-approves.

### 3.7 Bridge serialization for the blocking modals (MODIFY `Bridge`)

- **What:** add an `asyncio.Lock` so only one blocking modal (`ask`/`confirm`) is outstanding on the single screen at a time; concurrent blocking calls from different agents queue (or get a clean `busy`). `notify` stays non-blocking. Prevents two agents racing the one device screen now that multiple clients share the daemon.
